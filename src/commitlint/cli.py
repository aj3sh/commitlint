...
def _show_errors(
    ...
    if skip_detail:
        output.error(VALIDATION_FAILED)
        return

    output.error(f"✖ Found {error_count} error(s).")
    for error in errors:
...
def _get_commit_message_from_file(filepath: str) -> str:
    ...
    output.verbose(f"reading commit message from file {abs_filepath}")
...
def _handle_commit_message(commit_message: str, skip_detail: bool) -> None:
    ...
    if success:
        output.success(VALIDATION_SUCCESSFUL)
        return
...
def _handle_multiple_commit_messages(
    ...
    for commit_message in commit_messages:
        success, errors = lint_commit_message(commit_message, skip_detail=skip_detail)
        if success:
            output.verbose("lint success")
            continue

        output.verbose("lint failed")
        has_error = True
        _show_errors(commit_message, errors, skip_detail=skip_detail)
        output.error("")
...
def main() -> None:
    ...
    try:
        if args.file:
            ...
            output.verbose("checking commit message")
