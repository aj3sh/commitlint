...
def get_commit_message_of_hash(commit_hash: str) -> str:
    ...
    """
    output.verbose(f"fetching commit message from hash {commit_hash}")
    try:
        # Run 'git show --format=%B -s' command to get the commit message
        output.verbose(f"executing: git show --format=%B -s {commit_hash}")
        commit_message = subprocess.check_output(
            ["git", "show", "--format=%B", "-s", commit_hash],
            text=True,
            stderr=subprocess.PIPE,
        ).strip()
        output.verbose(commit_message)

        return commit_message
    except subprocess.CalledProcessError as ex:
        output.verbose("unable to fetch commit message using git command")
        output.verbose(f"{ex.__class__.__name__}: {ex}")
...
def get_commit_messages_of_hash_range(
    ...
    try:
        ...
        output.verbose(commit_messages_output)
        ...
        output.verbose("unable to fetch commit messages using git command")
        output.verbose(f"{ex.__class__.__name__}: {ex}")
